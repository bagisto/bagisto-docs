...
- We have created **`DataGrid`** abstract class in Webkul **`Ui`** package. In the DataGrid abstract class, a list of properties and methods are declared. So, while creating your own DataGrid you need to only extends the **`Webkul\Ui\DataGrid\DataGrid`** abstract class.

- In **`Webkul\Ui\DataGrid\DataGrid.php`** abstract class, two abstract methods are declared **`prepareQueryBuilder()`** and **`addColumns()`**. We can prepare grid by defining these two methods
      ...
      - Webkul/Blog/
        - src/
          ...
          - Datagrids/
            - PostDataGrid.php
    ```
- Add below code to your datagrid file i.e. **`PostDataGrid.php`**.
...
- Now go to **`Admin/PostController.php`**, there is an index method, just use the **`toJson`**.
